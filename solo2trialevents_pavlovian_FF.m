function TE = solo2trialevents_pavlovian_FF(filepath,ifsave)
%SOLO2TRIALEVENTS2_PAVLOVIAN_FF   Creates trial events structure.
%  The event are extracted from the saved behavior file.
%  Most of the events are given relative to the TrialStart event.
%
%   SOLO2TRIALEVENTS2_PAVLOVIAN_FF(SESSPATH,IFSAVE) uses an addidional
...
fs = strfind(filepath, '\');
sessionID = filepath(fs(end-1)+1:fs(end)-1);
animalID = filepath(fs(end-2)+1:fs(end-1)-1);

ntrials = SessionData.nTrials;

% Preallocation of space
[TE.NTrials TE.TrialStart TE.TrialEnd TE.TrialType TE.TrialType_r] = deal(nan(1,ntrials));
[TE.T1Ratio, TE.T2Ratio, TE.NoPunish] = deal(nan(1,ntrials));
...
for currentTrial = 1:ntrials
    ...
    TE.T1Ratio(1, currentTrial) = SessionData.TrialSettings(1).Type1;
    TE.T2Ratio(1, currentTrial) = SessionData.TrialSettings(1).Type2;
    TE.NoPunish(1, currentTrial) = SessionData.TrialSettings(1).NoPunish;
    TE.NTrials(1,currentTrial) = ntrials; %saving out the number of trials, just to be sure
    TE.Animalname{1,currentTrial} = animalID;
...
% % % 
% if SessionData.TrialSettings(1).DirectDelivery==0
% for currentTrial = 1:ntrials % Repair trial outcomes
%     if SessionData.TrialOutcome(currentTrial) == 2
%         SessionData.TrialOutcome(currentTrial) = 3;
%     end
% end
% end
...
for currentTrial = 20:(ntrials-1)
    ...
    if ~isempty(firstlickinx)   % only if there were licks after stimulus
        TE.RT(1,currentTrial) = rellicktimes(firstlickinx);
            ...
            if  (TE.Response(currentTrial) == 1) & (SessionData.TrialTypes(currentTrial)==1)
                TE.TrialType_r(currentTrial) = 1;
            elseif (TE.Response(currentTrial) == 1) & (SessionData.TrialTypes(currentTrial)==2)
                TE.TrialType_r(currentTrial) = 2;
            end
end
...
if ifsave == 1
    ...
    savename = [ animalID '_' sessionID];
    savename2 = 'TE';
    savename3 = 'TrialEvents';
